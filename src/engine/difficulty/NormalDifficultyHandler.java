...
public class NormalDifficultyHandler implements DifficultyHandler {
    ...
    public GameSettings adjustSettings(int formationWidth, int formationHeight, int baseSpeed, int shootingFrequency, int gameLevel) {
        ...
        if (isEvenLevelUnderFive || isLevelAtLeastFive) {
            int adjustBaseSpeed = isEvenLevelUnderFive ? 10 : 12; //5레벨 이상의 경우 baseSpeed 15 -> 12로 난이도 하향조정
            int adjustShootingFrequency = isEvenLevelUnderFive ? 200 : 230; //5레벨 이상의 경우 shootingFrequency 300 -> 230으로 난이도 하향조정

            if (formationWidth == formationHeight && formationWidth < 12) formationWidth += 1; //Width 증가 기준 14 -> 16 난이도 하향 조정
