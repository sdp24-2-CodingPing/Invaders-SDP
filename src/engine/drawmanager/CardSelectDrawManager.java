...
public class CardSelectDrawManager extends DrawManager {
  ...
  public static void drawCardSelectTitle(final Screen screen) {
    ...
    String instructionsString = "select with arrows, confirm with space";
  ...
  public static void drawCard(
    ...
    backBufferGraphics.fillRect(
        width + (int) (cardWidth * 0.05),
        height + (int) (cardHeight * 0.04),
        (int) (cardWidth * 0.9),
        (int) (cardHeight * 0.92));

    backBufferGraphics.drawImage(
        card.getCardImage(),
        width + cardWidth / 2 - imageWidth / 2,
        height + cardHeight / 4,
        imageWidth,
        imageHeight,
        null);
    ...
    backBufferGraphics.drawString(
        cardString,
        width + cardWidth / 2 - fontRegularMetrics.stringWidth(cardString) / 2,
        height + cardHeight * 3 / 4);
  }

  // TODO: Card Design 추가 수정 필요
  public static void drawCardList(
      final Screen screen, final ArrayList<Card> cardList, final int selectedCardIndex) {
    final int cardListSize = cardList.size();
    for (int i = 0; i < cardListSize; i++) {
      drawCard(
          screen,
          cardList.get(i),
          screen.getWidth() / 3 * i + screen.getWidth() / 24,
          screen.getHeight() / 5 * 3,
          i == selectedCardIndex);
    }
  }
}
