...
public class SettingDrawManager extends DrawManager {
  ...
  /** 볼륨 바를 그리는 메서드 */
  public static void drawVolumeBar(
      Screen screen, int x, int y, int totalWidth, int filledWidth, boolean isSelected) {
  ...
  /** 퍼센트 값을 그리는 메서드 */
  public static void drawVolumePercentage(
      Screen screen, int x, int y, int volume, boolean isSelected) {
  ...
  public static void drawShipBoxes(
      Screen screen, int x, int y, boolean isSelected, int index, boolean current) {
    if (current) {
      // Ship box
      backBufferGraphics.setColor(isSelected ? Color.GREEN : Color.WHITE);
      ...
      backBufferGraphics.drawString(
          PlayerShip.ShipType.values()[index].name(), x + index * 60 + 15, y + index * 20);
    } else {
      // Ship box
      backBufferGraphics.setColor(isSelected ? Color.GREEN : Color.WHITE);
      backBufferGraphics.fillRect(x + index * 60, y + index * 20, isSelected ? 0 : 10, 10);
  ...
  public static void drawCenteredRegularString(
      final Screen screen, final String string, final int height, boolean isSelected) {
    backBufferGraphics.setFont(fontRegular);
    // 선택된 경우 초록색, 그렇지 않으면 흰색으로 표시
    backBufferGraphics.setColor(isSelected ? Color.GREEN : Color.WHITE);
    backBufferGraphics.drawString(
