...
public class SettingScreen extends Screen {
  ...
  private void draw() {
    ...
    for (int i = 0; i < menuItems.length; i++) {
      ...
      SettingDrawManager.drawCenteredRegularString(
          this, menuItems[i], this.getHeight() / 3 + i * MENU_ITEM_GAP, isSelected);
    }
    ...
    SettingDrawManager.drawVolumeBar(
        this,
        this.getWidth() / 2 - VOLUME_BAR_WIDTH / 2,
        this.getHeight() / 3 + VOLUME_BAR_GAP,
        VOLUME_BAR_WIDTH,
        filledWidth,
        isVolumeSelected);

    SettingDrawManager.drawVolumePercentage(
        this,
        this.getWidth() / 2,
        this.getHeight() / 3 + VOLUME_BAR_GAP + VOLUME_PERCENTAGE_GAP,
        volumeLevel,
        isVolumeSelected);

    int NumberOfShips = PlayerShip.ShipType.values().length;
    for (int j = 0; j < NumberOfShips; j++) {
      SettingDrawManager.drawShipBoxes(
          this,
          this.getWidth() / 2 - 30 * NumberOfShips,
          this.getHeight() - 150,
          isShipChoiceSelected,
          j,
          j == this.currentShip);
    }
