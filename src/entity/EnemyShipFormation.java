...
public class EnemyShipFormation implements Iterable<EnemyShip> {
  ...
  public EnemyShipFormation(final GameSettings gameSettings, final GameState gameState) {
    ...
    for (List<EnemyShip> column : this.enemyShips) {
      for (int i = 0; i < this.nShipsHigh; i++) {
        if (i / (float) this.nShipsHigh < PROPORTION_E) spriteType = SpriteType.EnemyShipE1;
        ...
        else if (i / (float) this.nShipsHigh
            < PROPORTION_E + PROPORTION_D + PROPORTION_C + PROPORTION_B)
  ...
  public final void destroy(final EnemyShip destroyedShip, final float balance) {
    ...
    if (this.shooters.contains(destroyedShip)) {
      ...
      if (nextShooter != null) this.shooters.set(destroyedShipIndex, nextShooter);
      else {
  ...
  public final void applyDamageToEnemy(
      int damage, final EnemyShip damagedEnemy, final float balance) {
    for (List<EnemyShip> column : this.enemyShips) {
      for (int i = 0; i < column.size(); i++) {
        ...
        if (ship != null && ship.equals(damagedEnemy)) {
          ...
          if (damagedEnemy.getHealth() <= 0) {
            ...
            this.logger.info(
                "Destroyed ship in (" + this.enemyShips.indexOf(column) + "," + i + ")");
  ...
  public final Iterator<EnemyShip> iterator() {
    ...
    for (List<EnemyShip> column : this.enemyShips)
      for (EnemyShip enemyShip : column) enemyShipsList.add(enemyShip);
  ...
  public int getDistroyedship() {
    return distroyedship;
  }
