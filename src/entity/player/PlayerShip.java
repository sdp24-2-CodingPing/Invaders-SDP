...
public abstract class PlayerShip extends Entity {
  ...
  protected PlayerShip(
      ...
      final int positionY,
      final String name,
      final ShipMultipliers multipliers,
  ...
  public final boolean shoot(final Set<Bullet> bullets, float balance) {
    if (this.shootingCooldown.checkFinished()) {
      ...
      switch (this.getBulletCount()) {
        case 1:
          bullets.add(
              BulletPool.getBullet(positionX + this.width / 2, positionY, this.getBulletSpeed()));
          soundManager.playSound(Sound.PLAYER_LASER, balance);
          break;
        case 2:
          bullets.add(
              BulletPool.getBullet(positionX + this.width, positionY, this.getBulletSpeed()));
        ...
        default:
          bullets.add(
              BulletPool.getBullet(positionX + this.width, positionY, this.getBulletSpeed()));
          bullets.add(BulletPool.getBullet(positionX, positionY, this.getBulletSpeed()));
          for (int i = 1; i <= this.getBulletCount() - 2; i++)
            bullets.add(
                BulletPool.getBullet(
                    positionX + this.width * i / (this.getBulletCount() - 1),
                    positionY,
                    this.getBulletSpeed()));
  ...
  public final void update() {
    if (!this.destructionCooldown.checkFinished()) this.spriteType = SpriteType.ShipDestroyed;
    else this.spriteType = this.baseSprite;
  }

  /**
   * Player HP decrease by received damage
   *
   * @param damage received damage value
   * @param balance sound balance (for two player mode)
   */
   ...
   *
   * @param heal received heal value
   */
  ...
  public final int getBulletSpeed() {
    return Math.round(
        (BULLET_SPEED - playerCardStatus.getBulletsSpeedLevel()) * this.multipliers.bulletSpeed());
  }
  ...
  public final int getShootingInterval() {
    return (int)
        Math.round(
            SHOOTING_INTERVAL
                * (Math.pow(0.9, (double) playerCardStatus.getIntervalLevel()))
                * this.multipliers.shootingInterval());
  }

  /**
   * Calculates and returns the player's attack power. The attack power can depend on player's
   * level, upgrades, or items.
   *
   * @return The calculated attack power.
   */
  public final int getPlayerAttackDamage() {
    return (Math.round(
        (ATTACK_DAMAGE + playerCardStatus.getAttackDamageLevel() * 5) * this.multipliers.damage()));
  }
