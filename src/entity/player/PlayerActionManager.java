...
public class PlayerActionManager {
  ...
  public void manageMovement(int playerNumber, int screenWidth, float balance, List<Web> webs) {
    ...
    boolean isRightBorder =
        this.playerShip.getPositionX() + this.playerShip.getWidth() + this.playerShip.getSpeed()
            > screenWidth - 1;
    boolean isLeftBorder = this.playerShip.getPositionX() - this.playerShip.getSpeed() < 1;
  ...
  public void manageShooting(Set<Bullet> bullets) {
    ...
    if (player1Attacking && this.playerShip.shoot(bullets)) {
      this.gameState.setBulletsShot(
          this.gameState.getBulletsShot() + this.playerShip.getBulletCount());
    }
  }
}
